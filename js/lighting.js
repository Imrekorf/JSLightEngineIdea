...
var Pathways, Lights;
// drag node functions
  function drag_light_listener(id){
    ...
    function movemouse(e) {
      var div = document.getElementById(id);
      getDis = false;
      if(e.clientY-offY > 10) {
        div.style.top = (e.clientY-offY) + 'px';
        getDis = true;
      }
      if(e.clientX-offX > 10) {
...
// create array for nodes
  function CreatePathway() {
    Pathways2 = document.getElementsByClassName('body_parent');
    Pathways = new Array();

    for(var i = 0; i < Pathways2.length; i++){
      Pathways[i] = new pathway(Pathways2[i].id);
    }
  }

  function CreateLights() {
    Lights2 = document.getElementsByClassName('light');
    Lights = new Array();

    for(var i = 0; i < Lights2.length; i++){
      Lights[i] = new Light(Lights2[i].id);
    }
  }
    ...
    getLight() {
      for(var j = 0; j < this.childs.length; j++){
        ...
        for(var i = 0; i < lights.length; i ++){
          ...
          var dis1 = map(Math.abs(Math.sqrt( Math.pow(x2 - x, 2) + ( Math.pow(y2 - y, 2) ))), 0, lights[i].brightness*10, 0, 100);
        ...
        var f1 = map(contot1, 0, lights.length, 0.01, 3);
